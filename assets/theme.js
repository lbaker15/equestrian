...
window.addEventListener('DOMContentLoaded', () => {
  ...
  let burgerLines = Array.from(document.querySelectorAll('.mphus-burger__line'));
  let headerItem = Array.from(document.querySelectorAll('.header__item'));
  let headerChildrenMenu = document.querySelector('.header__menu-children');
  let headerLogo = document.querySelector('.header__logo');
  let headerParentMenu = document.querySelector('.header__menu-parents')
  let headerParent = Array.from(document.querySelectorAll('.header__parent'));
  let headerBack = Array.from(document.querySelectorAll('#header-back'));
  let titleMenu = document.querySelector('#html');
  let screenWidth;
  ...
  headerBack.forEach(item => {
    item.addEventListener('click', () => {
      headerParentMenu.style.display = 'flex'
      headerChildrenMenu.style.display = 'none'
    })
  })
  headerItem.forEach(item => {
    item.addEventListener('click', (e) => {
      // if (e.target.dataset.value !== 'shop_all') {
        let allChildren = Array.from(headerChildrenMenu.querySelectorAll('.header__children'));
        headerParentMenu.style.display = 'none'
        headerChildrenMenu.style.display = 'flex'
        let currValue = e.target.dataset.value;
        allChildren.forEach(c => c.classList.remove('child-active'))
        let children = allChildren.filter(item => item.dataset.value === currValue)
        if (children.length > 0) {
          children.forEach((child, index) => {
            child.classList.add('child-active')
          })
        } else {}
        titleMenu.innerHTML = String(e.target.dataset.id)
      // }
    })
  })
  //BURGER MENU
  ...
  if (prodCart) {
    prodCart.addEventListener('click', async () => {
      let inventory = Number(selectedV.dataset.value);
      let id = Number(selectedV.value);
      let quantity = 1;

      let cart = await getCart()
      Promise.all(cart.items.filter(item => {
          return item.variant_id === id;
        }))
        .then(inCart => {
          //If no inCart[0] then the variant is not in the cart
          let quan = (inCart[0]) ? inCart[0].quantity : 0;
          if (quan + 1 <= inventory) {
            let data = {
              id: Number(id),
            ...
            fetch('/cart/add.js', {
              ...
              /* we have JSON */
              if (json) {
                added.innerHTML = `Added to Cart <img src="https://cdn.shopify.com/s/files/1/0629/7888/3813/files/BAG.png?v=1652728986" />`
                add.style.display = 'none';
                added.style.display = 'flex';

                let newCart = await getCart()
                document.querySelector('.header__cart-count').innerHTML = newCart.item_count
              }
            }).catch(function (err) {
              console.error(err)
            });
          } else {
            added.innerHTML = 'Not enough stock'
            add.style.display = 'none';
            added.style.display = 'flex';
          }
        })
  ...
  let prodHeroCarouselIn = document.querySelector('.product__hero-carousel-inner');
  ...
  let prodHeroImgX = document.querySelector('.product__hero-img-overlay-x');
  ...
  let carouselMove = document.querySelector('.product__hero-carousel-move');
  let tlOverlay = gsap.timeline({
    paused: true
  })
  tlOverlay.fromTo(prodHeroImgOverlay, {
    display: 'none'
  }, {
    display: 'flex'
  })
  ...
  tlOverlay.fromTo(document.body, {
    overflowY: 'scroll'
  }, {
    overflowY: 'hidden'
  }, '<')
  ...
  let count = 1;
  // let scrollGap = getComputedStyle(prodHeroCarouselIn).getPropertyValue('column-gap');
  // let scrollNumber = Number(scrollGap.split('px')[0]) + 5;
  // let grandWidth = (prodImgCarousel[0].width + scrollNumber);
  // console.log(grandWidth);
  // gsap.set('.product__hero-carousel-inner', {
  //   width: (prodImgCarousel.length * grandWidth ) - (grandWidth / 2)
  // })
  ...
  // let scrollGap = 30;
  // prodHeroCarouselIn.style.columnGap = scrollGap;
  const scrollerFunc = () => {
    let len = prodImgCarousel.length;
    let length = 100 / len;
    let scrollGap = getComputedStyle(prodHeroCarouselIn).getPropertyValue('column-gap');
    let scrollNumber = Number(scrollGap.split('px')[0]) + 5
    let val = (count === len) ? ((count) * length) + scrollNumber : (count) * length
    gsap.to(scroller, {
      width: val + '%'
    })
  }
  const theLine = (mobile) => {
    let imgWidth = prodImgCarousel[0].getBoundingClientRect().width;
    let active = prodImgCarousel[count - 1]
    prodImgCarousel.forEach(item => item.classList.remove('img-active'))
    active.classList.add('img-active')
    if (!mobile) {
      prodHeroCarouselIn.scrollLeft = (count - 1) * (imgWidth + 20);
    }
  }
  ...
  if (prodImgCarousel.length) {
    prodImgCarousel.map((item, i) => {
      item.addEventListener('click', (e) => {
        ...
        let scrollGap = getComputedStyle(prodHeroCarouselIn).getPropertyValue('column-gap');
        let scrollNumber = Number(scrollGap.split('px')[0]) + 5
        let val = (i === len - 1) ? ((i + 1) * length) + scrollNumber : (i + 1) * length;
        gsap.to(scroller, {
          width: val + '%'
        })
        theLine()
  ...
  if (prodHeroImgX) {
    prodHeroImgX.addEventListener('click', () => {
      tlOverlay.reverse()
      prodImgOverlay = false;
    })
  }
  ...
  let filterOpen = Array.from(document.querySelectorAll('.filter__open'))
  let filterOptions = Array.from(document.querySelectorAll('.filter__options'))
  let filterOptionSng = Array.from(document.querySelectorAll('.filter__option'))
  let filterOpenArr = [true, true]
  filterOpen.map((f, i) => {
    f.addEventListener('click', () => {
      if (filterOpenArr[i]) {
        filterOpenArr[i] = false
        filterOptions[i].style.display = 'flex'
        console.log(filterOpenArr, filterOptions[i])
      } else if (!filterOpenArr[i]) {
        filterOpenArr[i] = true
        filterOptions[i].style.display = 'none'
      }

    })
  })
  filterOptionSng.map((item, i) => {
    item.addEventListener('click', (e) => {
      let tag = String(e.target.dataset.value).toLocaleLowerCase();
      let currentPath = window.location.pathname;
      let locationOrigin = window.location.origin;
      let filterSanitize = tag.replace(/\s+/g, '-')
      console.log('cl', currentPath, locationOrigin)
      console.log('fs', filterSanitize)
      if (currentPath.indexOf(filterSanitize) !== -1) {
        console.log('already in')
        // var newPath = currentPath.replace(filterSanitize, '')
        // newPath = newPath.replace('/+', '/').replace('++', '+').replace(/\+$/, '').replace(/\/$/, '')
        // newUrl = locationOrigin + newPath
        // window.location.href = newUrl
      } else {
        console.log('not in')
        // var collPath = 'collections/{{collection.handle}}'
        // var endOfPath = currentPath.substring(currentPath.length - collPath.length - 2)
        // if (endOfPath.indexOf(collPath) == -1) {
        //   newUrl = locationOrigin + currentPath + "+" + filterSanitize
        // } else {
